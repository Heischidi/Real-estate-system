"""
Private Property Nigeria scraper.

Target: https://www.privateproperty.com.ng
...
class PrivatePropertyScraper(BaseScraper):
    """Scraper for privateproperty.com.ng."""
    ...
    def base_url(self) -> str:
        return "https://www.privateproperty.com.ng"
    ...
    async def _scrape_city(self, browser: Browser, city_slug: str, city_enum: City) -> list[ListingData]:
        ...
        for page_num in range(1, 3):
            url = f"{self.base_url}/for-sale/{city_slug}?p={page_num}"
            html = await self._get_page_content(url, browser, wait_for_selector=".listings-section")
            if not html:
                break
    ...
    def _parse_card(self, card: BeautifulSoup, city: City) -> ListingData | None:  # type: ignore[name-defined]
        ...
        title_tag = card.select_one("h2, h3, h4, [class*='title']")
        ...
        loc_tag = card.select_one("[class*='address'], [class*='location']")
...
_registry["privateproperty"] = PrivatePropertyScraper
